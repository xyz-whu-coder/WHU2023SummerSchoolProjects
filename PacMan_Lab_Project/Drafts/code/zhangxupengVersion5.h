...
int kill_op(struct Player* player, int x, int y) {
	...
	if (x == xx && y == yy&& player->your_status > 1 && player->opponent_status == 1) 
		return 1;
	return 0;
}
...
int  evaluate_dis(int ex, int ey, int x, int y) {//A* 估值函数
	return abs(ex - x) + abs(ey - y);  //曼哈顿距离
}
void find_dis(struct Player* player, struct PW* pos, int ex, int ey, int sx, int sy, int vis[][20], int dis) { //dfs搜索距离目标的最短距离 
	//A*剪枝
	if (dis + evaluate_dis(ex, ey, sx, sy) > pos->d)return;
...
void tag_superstar(struct Player* player) {  //标记超级星位置
...
//	return flag;
//}
int   seek_ghost(struct Player* player) {    //判断当前状态，如果强化状态，且有可能击杀对手，就设置目标去kill opponent
	struct Point ret = { player->your_posx, player->your_posy };  //当前位置	
...
void seek_star(struct Player* player, int x, int y, struct PW* pos, int dis, int vis[][20]) {  //dfs搜索和最短距离的星星的位置  
	if (dis > pos->d)return;
