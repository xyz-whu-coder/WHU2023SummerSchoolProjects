...
struct Point walk(struct Player *player)
{
    ...
    step++;
    if (player->opponent_status == -1 && player->your_score - 400 - 1 > player->opponent_score)
    ...
    if (stat > 1)
    {
        for (int di = 0; di < 4; ++di)
        {
            ...
            if (!ava(mn.x, mn.y, player))
                continue;
            if (g1.x == mn.x && g1.y == mn.y)
                return makepoint(mn.x, mn.y);
            if (g0.x == mn.x && g0.y == mn.y)
                return makepoint(mn.x, mn.y);
        }
    }
    for (int i = 0; i < n; ++i)
    ...
    if (op_stat && player->your_score > 0)
        danger = min(danger, d[me.x][me.y][op.x][op.y]);
    // if (step < 5)
