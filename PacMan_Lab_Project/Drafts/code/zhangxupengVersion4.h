...
void init(struct Player *player)
{
	...
	mind = SUPERSTAR;
	// next_star.X = player->your_posx;
	// next_star.Y = player->your_posy;
}
...
{ // dfs搜索和最短距离的星星的位置
	if (dis > pos->d)
		return;
	if ((player->mat[x][y] == 'o' || player->mat[x][y] == 'O') && pos->d > dis)
	{
...
struct Point walk(struct Player *player)
{
	seek_op(player);
	struct Point ret = {player->your_posx, player->your_posy}; // 当前位置
	struct PW pos = {ret, 100000};
	struct Point next = ret; // 下一步走的地方 初始化为原地不动
	int xx, yy;
	int min_dis = 10000;
	// 判断是否到达要找的星星或者对手
	if (mind == SUPERSTAR && next_star.X == ret.X && next_star.Y == ret.Y)
	{
		superstar_cnt--;
	}
	if ((mind == STAR || mind == SUPERSTAR) && next_star.X == ret.X && next_star.Y == ret.Y || mind == KILL && player->opponent_status == -1)
	{
		for (int i = 0; i < 4; i++)
		{
