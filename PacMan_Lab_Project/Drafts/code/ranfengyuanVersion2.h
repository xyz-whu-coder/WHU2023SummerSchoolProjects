...
{
    return x == player->opponent_posx && y == player->opponent_posy && player->opponent_score >= 200 && player->opponent_status == 0;
}
int min_dis;
int vis[100][100];
int flag;
void nearSuperStar(struct Player *player, int x, int y, int dis)
{
    ...
    if (min_dis < dis || !isValid(player, x, y) || vis[x][y])
        return;
    if (player->your_status > 0 && isMeetingOpponent(player, x, y) && (player->your_status - dis) >= 0)
    {
        min_dis = dis;
        return;
    }
    else if (player->your_status > 0 && isMeetingGhost(player, x, y) && (player->your_status - dis) >= 0)
    {
        min_dis = dis;
        return;
    }
    else if (player->mat[x][y] == SUPER_STAR)
    {
        min_dis = dis;
        return;
    }
